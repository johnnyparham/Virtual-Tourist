...
//  Virtual Tourist
//
//  Created by Johnny Parham on 8/11/16.
    ...
    func downloadPhotosForPin(pin: Pin, completionHandler: (success: Bool, error: NSError?) -> Void) {
        ...
        if let numberPages = pin.pageNumber?.integerValue {
            if numberPages > 0 {
                ...
                randomPageNumber =
                   Int(arc4random_uniform(UInt32(pageLimit))) + 1
            }
        }
        ...
        // Mark GET request for get photos for pin
        taskForGETMethodWithParameters(parameters, completionHandler: {
            ...
            if let error = error {
                ...
                // Response dictionary
                if let photosDictionary = results.valueForKey(JSONResponseKeys.Photos) as? [String:AnyObject], photosArray = photosDictionary [JSONResponseKeys.Photo] as? [[String : AnyObject]], numberOfPhotoPages = photosDictionary [JSONResponseKeys.Pages] as? Int {
